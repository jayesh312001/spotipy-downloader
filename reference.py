...
def task(tracks):
    ...
    for i in tracks:
        ...
        try:
            ...
            vid_url = "http://youtu.be" + results[0]["url_suffix"].replace(
                "watch?v=", ""
            )
        ...
        try:
            ...
            webm = AudioSegment.from_file(
                ospath.join(download_path, yt.default_filename)
            )
            ...
            export_song.export(
                ospath.join(
                    download_path,
                    str(j["artists"][0]["name"] + "-" + j["name"] + ".mp3").replace(
                        '"', ""
                    ),
                ),
                format="mp3",
            ...
            request.urlretrieve(
                ...
                ospath.join(
                    download_path,
                    str(j["artists"][0]["name"] + "-" + j["name"] + ".jpg").replace(
                        '"', ""
                    ),
                ),
            )
...
if __name__ == "__main__":
    ...
    lbl2 = Label(
        window, text="Output:", font=("Arial Bold", 12), bg="#3d3d3d", fg="white"
    )
    ...
    lbl3 = Label(
        window, text="Audio Quality:", font=("Arial Bold", 9), bg="#3d3d3d", fg="white"
    )
    lbl3.place(relx=0.14, rely=0.78, anchor=CENTER)

    lbl4 = Label(
        window,
        text="Download location: " + str(ospath.join(application_path, "Downloads")),
        font=("Arial Bold", 9),
        bg="#3d3d3d",
        fg="white",
    )
    ...
    scrolled = scrolledtext.ScrolledText(
        window, width=55, height=15, font=("Arial", 10)
    )
